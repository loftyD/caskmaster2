...
function formatForTextArea($string)
{
...
function convertFieldsetToCssClass($str, $replace=array(), $delimiter='-') {
	if( !empty($replace) ) {
...
function surround($s,$char="'") {

return $char.$s.$char ;

}

static function convertColNameToString($str) {
return self::convertTableNameToString($str) ;
}

static function convertTableNameToString($str) {
$label = explode("_",$str);
...
function validateUKDate($date, $format = 'd/m/Y')
{
    $d = DateTime::createFromFormat($format, $date);
    return $d && $d->format($format) == $date;
}

function convertUKDateToMySql($date)
{
	$d = explode("/",$date);
